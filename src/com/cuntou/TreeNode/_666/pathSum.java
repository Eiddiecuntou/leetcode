package com.cuntou.TreeNode._666;

/**
 * @ClassName : pathSum  //类名
 * @Description :   //描述
 * @Author : 村头 //作者
 * @Date: 2022/4/27  19:59
 */
...
public class pathSum {
    ...
    public int pathSum(int[] nums) {
        ...
        for (int num : nums) {
            ...
            int shi = num % 100 / 10;
            int ge = num % 10;
            //因为每一行的第一个都是使用的 2^n -1 这样子来表示下标的
    ...
    private void dfs(Integer[] tree, int i, int currPathSum) {
        ...
        currPathSum += tree[i];
        if ((tree[2 * i + 1] == null && tree[2 * i + 2] == null) || i >= 7) {
            ans += currPathSum;
            return;
        }

        dfs(tree,2 * i + 1,currPathSum);
        dfs(tree,i * 2 + 2,currPathSum);
    }
